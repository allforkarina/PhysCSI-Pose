...
def analyze_csi(
    ...
    *,
...
) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], dict[tuple[int, int], CsiSequence]]:
    ...
    for idx, path in enumerate(selected, start=1):
        ...
        for name, arr in data.items():
            ...
            if name == expected_key:
                finite = np.isfinite(arr) if np.issubdtype(arr.dtype, np.number) else np.array([True])
    ...
    return summary, sequence_rows, file_issues, sequences
...
def update_keypoint_stats(gt: np.ndarray, keypoint_stats: list[dict[str, NumericSummary]], rng: np.random.Generator) -> None:
    ...
    for joint in range(min(gt.shape[1], len(keypoint_stats))):
        ...
        keypoint_stats[joint]["conf"].update(joint_conf[np.isfinite(joint_conf)], rng)
...
def bbox_and_motion_stats(gt: np.ndarray, bbox_stats: dict[str, NumericSummary], motion_stats: NumericSummary, rng: np.random.Generator) -> None:
    ...
    conf = gt[..., 2].astype(np.float64, copy=False)
    valid = np.isfinite(xy).all(axis=-1) & ~np.all(xy == 0.0, axis=-1) & np.isfinite(conf) & (conf > 0.0)
...
def analyze_gt(
    ...
    *,
    ...
    sample_limit: int,
    rng: np.random.Generator,
) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]], dict[tuple[int, int], dict[str, Any]], list[dict[str, Any]]]:
    selected = files[:max_files] if max_files is not None else files
    ...
    coord_stats = {
        ...
        "conf": NumericSummary(sample_limit=sample_limit),
    }
    bbox_stats = {
    ...
    for idx, path in enumerate(selected, start=1):
        ...
        if bool(invalid_conf.any()):
            issue_bits.append("nonfinite_conf")
        if bool(conf_out_of_range.any()):
            issue_bits.append("conf_out_of_range")
            ...
            issue_rows.append(
                {
                    ...
                    "invalid_conf": int(invalid_conf.sum()),
                    "conf_out_of_range": int(conf_out_of_range.sum()),
                }
            )
        ...
        row = {
            ...
            "gt_shape": shape_text(shape),
            "gt_dtype": str(gt.dtype),
            "valid_keypoints": int(valid_xy.sum()),
            "zero_xy_keypoints": int(zero_xy.sum()),
            "invalid_xy_keypoints": int(invalid_xy.sum()),
            "invalid_conf_keypoints": int(invalid_conf.sum()),
            "conf_out_of_range_keypoints": int(conf_out_of_range.sum()),
        ...
        keypoint_rows.append(
            {
                ...
                "conf": json.dumps(stats["conf"].as_dict(), sort_keys=True),
            }
        )
    ...
    return summary, sequence_rows, issue_rows, gt_by_subject_action, keypoint_rows
...
def main() -> None:
    ...
    csi_summary, csi_rows, csi_file_issues, csi_sequences = analyze_csi(
        csi_root,
        csi_files,
    ...
    gt_summary, gt_rows, gt_file_issues, gt_by_key, keypoint_rows = analyze_gt(
        gt_root,
        gt_files,
    ...
    write_csv(output_root / "gt_keypoint_stats.csv", keypoint_rows)
