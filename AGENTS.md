...
Source-only runs require exactly one `--source-envs` value. After environment filtering, every subject's frames are deterministically shuffled with the training seed and split into 70% train, 10% validation, and 20% test. This random frame-level protocol is the only project train/validation/test split; no subject-disjoint split is exposed. Training logs print the three frame counts, and evaluation reconstructs the same membership from `--eval-envs` and `--eval-split`.
...
Run explicit CSI domain alignment when testing whether cross-domain degradation is caused by source/target axial CSI feature covariance mismatch. This keeps supervised pose losses on source and few-shot target samples while adding CORAL alignment between source and target axial encoder features:

```powershell
# Axial CORAL alignment: adapt CSI feature extraction while preserving pose supervision
python train.py --mode finetune_align --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\ft_align_coral_w01 --finetune-from outputs\source_baseline\best_val_mpjpe.pth --trainable-groups encoder --align-loss coral --align-layer axial --align-weight 0.1 --few-shot-subjects 4 --few-shot-frames 5 --epochs 30
```
...
python train.py --mode finetune --dataset-root data\mmfi_pose --target-envs env2 --output-dir outputs\ft_lower_limb_w15 --finetune-from outputs\source_baseline\best_val_mpjpe.pth --trainable-groups encoder --joint-loss-preset lower_limb --lower-limb-weight 1.5 --few-shot-subjects 4 --few-shot-frames 10 --epochs 50 --batch-size 64
```
